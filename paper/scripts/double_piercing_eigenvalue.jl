...
I = findall(F.values) do λ
    3 < real(λ) < 5 && -3e-1 < imag(λ) < 0
end
i = I[1]
scatter!(ax2,[real(F.values[i])],[imag(F.values[i])],markersize=7,color=:red,label="")
...
vlines!(ax1,[-pml_start,pml_start];ymin=0,ymax=1, linestyle=:dash, color=(:black,0.5))
