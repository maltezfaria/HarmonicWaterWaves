...
P = 5
h = P*λ / ppw
meshsize = Dict(ent => h for ent in union(HWW.freesurface(tank),HWW.bottom(tank),HWW.obstacles(tank)))
...
if norm(ee1,Inf) > 1e-10
    @warn "Incident wave does not satify surface bottom conditions" norm(ee1,Inf), norm(ee2,Inf)
end
...
h1 = heatmap!(ax1,xrange,yrange,lift(t -> real.(exp(-im*t)*ϕ_inc),time),colorrange=(cmin,cmax))
lines!(ax1,tank.mesh)
...
h2 = heatmap!(ax2,xrange,yrange,lift(t->real.(exp(-im*t)*ϕ_tot),time),colorrange=(cmin,cmax))
vlines!(ax2,[-pml_start,pml_start];ymin=0.33,ymax=1, linestyle=:dash, color=(:black,0.5))
lines!(ax2,tank.mesh)
...
h3 = heatmap!(ax3,xrange,yrange,lift(t -> real.(exp(-im*t)*ϕ_sca),time),colorrange=(cmin,cmax))
vlines!(ax3,[-pml_start,pml_start];ymin=0.33,ymax=1, linestyle=:dash, color=(:black,0.5))
lines!(ax3,tank.mesh)

cb = Colorbar(fig[1:3, 2];
    limits=(cmin,cmax),
    height=Relative(0.8),
    width = 10,
...
if make_animation
    ...
    record(fig, "paper/animations/jellyfish_fields.gif", timestamps;
