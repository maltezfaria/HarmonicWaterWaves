...
fig = Figure()
ax  = Axis(fig[1, 1]; yscale = log10, xlabel = L"(M-a)/\lambda", ylabel = L"E_{M,h}", limits = (0, last(ll), 1e-12, 1), yminorticksvisible = false)

for ν in impedances
    vals = dict[ν]
    sc = scatterlines!(ax, ll, vals.er; marker = :circle, label = L"\nu = %$(ν)")
end
...
lines!(
    ax,
    ll,
    dict[impedances[end]].er[iref] / ref[iref] * ref;
