...
for i ∈ idxs_wall ∪ idxs_bottom
    f[i] = dϕi(tank.quad.qnodes[i])
end
@info norm([dϕi(q) + ν*ϕi(q) for q ∈ tank.quad.qnodes[idxs_free]], Inf)
@info norm([dϕi(q) for q ∈ tank.quad.qnodes[idxs_bottom]], Inf)

# solution
ϕ_pml = HWW.solve!(tank, f)
## Plot exact solution and its analytic extension
Γf = HWW.freesurface(tank)
xx = [dof.coords[1] for dof ∈ quad.qnodes]
# fig = plot(tank,aspect_ratio=4)
fig = Figure()
ax = Axis(fig[1, 1]; xlabel = L"x_1/\lambda", limits = (0, domain_end/λ, -1.1, 2.1))
xrange = 0:0.01:domain_end
ϕ_exact = [ϕi((x, 0)) for x ∈ xrange]
ϕ_pml_exact = [ϕi(τ((x, 0))) for x ∈ xrange]
lines!(ax, xrange./λ, real.(ϕ_exact); label = L"$\mathrm{Re}(\varphi)$", color = Cycled(1))
lines!(
    ax,
    xrange./λ,
    real(ϕ_pml_exact);
